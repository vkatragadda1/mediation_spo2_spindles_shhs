# SpO2–Spindle–Dementia Stratification: BY SEX 
...
# Load data. Replace name with CSV file. Run females first, then males. 
...
# Define covariate groups. Sex is NOT used as a covariate here. 
covariate_sets <- list(
  Unadjusted = c(),
  Group1 = c("age", "bmi"),
  Group2 = c("age", "bmi", "smoking", "alcohol", "education"),
  Group3 = c("age", "bmi", "smoking", "alcohol", "education", "group1_med", "group2_med")
)
...
for (stage in names(covariate_sets)) {
  # Set covariates for this stage and initialize results dataframe 
